...
void PlayerRegistration::loadPlayers(const char *filename)
{
    ...
    while (getline(file, line) && totalPlayers < 100)
    {
        ...
        getline(ss, token, ',');
        strcpy(p.playerID, token.c_str());
        getline(ss, token, ',');
        strcpy(p.name, token.c_str());
        getline(ss, token, ',');
        strcpy(p.university, token.c_str());
        getline(ss, token, ',');
        p.ranking = stoi(token);
        getline(ss, token, ',');
        strcpy(p.status, token.c_str());
        getline(ss, token, ',');
        p.matchesPlayed = stoi(token);
        getline(ss, token, ',');
        p.points = stoi(token);
        getline(ss, token, ',');
        strcpy(p.grouping, token.c_str());
